		  ...
		  <div class="table-responsive">
			...
			<?php $dbend = $row['availableto'].' '.$row['clock_end'];?>
				<tr>
					...
					<?php if(date('m/d/Y H:i') < $dbstart && $this->db->get_where('student_question',array('exam_code'=>$row['exam_code'],'student_id'=>$this->session->userdata('login_user_id')))->row()->answered != 'answered'):?>
						<a class="btn nc btn-rounded btn-sm btn-warning" style="color:white"><?php echo get_phrase('no_start');?></a>
					<?php endif;?>
					<?php if(date('m/d/Y H:i') >= $dbend &&  $this->db->get_where('student_question',array('exam_code'=>$row['exam_code'],'student_id'=>$this->session->userdata('login_user_id')))->row()->answered != 'answered'):?>
						<a class="btn nc btn-rounded btn-sm btn-danger" style="color:white"><?php echo get_phrase('exam_finish');?></a>
					<?php endif;?>
					<?php if($this->db->get_where('student_question',array('exam_code'=>$row['exam_code'],'student_id'=>$this->session->userdata('login_user_id')))->row()->answered != 'answered' && $query > 0 && date('m/d/Y H:i') >= $dbstart && date('m/d/Y H:i') < $dbend):?><a class="btn btn-rounded btn-sm btn-success" href="<?php echo base_url();?>student/examroom/<?php echo $row['exam_code'];?>"><?php echo get_phrase('take_exam');?></a>
					<?php endif;?>
